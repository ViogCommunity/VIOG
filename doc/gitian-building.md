...
*Setup instructions for a Gitian build of Vivoinnovaonexgobyte using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Vivoinnovaonexgobyte
...
- [Building Vivoinnovaonexgobyte](#building-vivoinnovaonexgobyte)
...
Clone the git repositories for vivoinnovaonexgobyte and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Iwwerall/VIOG
```
...
in the vivoinnovaonexgobyte repository under 'Fetch and build inputs' to install sources which require
...
To build Vivoinnovaonexgobyte (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the vivoinnovaonexgobyte repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vivoinnovaonexgobyte/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Iwwerall/VIOG
...
For example:
```bash
URL=https://github.com/crowning-/vivoinnovaonexgobyte.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vivoinnovaonexgobyte=${COMMIT} --url vivoinnovaonexgobyte=${URL} ../vivoinnovaonexgobyte/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vivoinnovaonexgobyte=${COMMIT} --url vivoinnovaonexgobyte=${URL} ../vivoinnovaonexgobyte/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vivoinnovaonexgobyte=${COMMIT} --url vivoinnovaonexgobyte=${URL} ../vivoinnovaonexgobyte/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the vivoinnovaonexgobyte git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../vivoinnovaonexgobyte/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/vivoinnovaonexgobyte.git
SIGPATH=/some/root/path/vivoinnovaonexgobyte-detached-sigs.git

./bin/gbuild --url VIOG=${BTCPATH},signature=${SIGPATH} ../VIOG/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vivoinnovaonexgobyte-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vivoinnovaonexgobyte-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vivoinnovaonexgobyte-osx-build.assert
```
