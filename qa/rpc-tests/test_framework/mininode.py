# mininode.py - Vivoinnovaonexgobyte P2P network half-a-node
#
...
#
# NodeConn: an object which manages p2p connectivity to a vivoinnovaonexgobyte node
# NodeConnCB: a base class that describes the interface for receiving
#             callbacks with network messages from a NodeConn
# CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
#     data structures that should map to corresponding structures in
#     vivoinnovaonexgobyte/primitives
...
import vivoinnovaonexgobyte_hash
...
def vivoinnovaonexgobytehash(s):
    return vivoinnovaonexgobyte_hash.getPoWHash(s)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(vivoinnovaonexgobytehash(r))
            self.hash = vivoinnovaonexgobytehash(r)[::-1].encode('hex_codec')
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1):
        ...
        print 'MiniNode: Connecting to Vivoinnovaonexgobyte Node IP # ' + dstaddr + ':' \
            + str(dstport)
